context("default parameters test")
...
testthat::test_that("Test for default parameters (ans extra_parameters too)", {
  ans=gen_test_db()
  ...
  formatter<-function(varcase_txt, context_df, nalabel, subset_df, short=FALSE, ...) {
    ...
    ans<-paste0(ans, " with ", length(unique(context_df$colour)), " distinct colours: ", paste0(unique(context_df$colour), collapse = ', '))
    return(ans)
  }
  #debugonce(typeReporter_factory)
  rap_fun<-typeReporter_factory(reportClass = rcvar, type = 'label_na', type_caption = 'NA labels of labelled', extra_parameters = list(colour=quote(expr=)),
                                formatters=formatter, flag_use_case=TRUE, colour='red')
